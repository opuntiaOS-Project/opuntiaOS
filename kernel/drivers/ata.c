...
void init_ata(ata_t *ata, uint8_t port, char is_master){
    ata->is_master = is_master;
    ata->data_port = port;
    ata->sector_count_port = port + 0x1;
    ata->lba_lo_port = port + 0x2;
    ata->lba_mid_port = port + 0x3;
    ata->lba_hi_port = port + 0x4;
    ata->device_port = port + 0x5;
    ata->device_port = port + 0x6;
    ata->device_port = port + 0x7;
    ata->device_port = port + 0x206;
} 

void indentify_ata_device(ata_t *ata) {
    port_16bit_out(ata->data_port, ata->is_master ? 0xA0 : 0xB0);
    port_8bit_out(ata->control_port, 0);
    
    port_16bit_out(ata->data_port, 0xA0);
    uint8_t status = port_8bit_in(ata->control_port);
    if (status == 0xFF) {
        printf("Err1");
        return;
    }
