...
void IconView::display(const LG::Rect& rect)
{
    ...
    const int underline_y = AppListView::icon_view_size() - underline_height() - padding;
    if (entity().windows().size() > 1) {
        const int len = 10;
        ctx.fill({ (AppListView::icon_view_size() - len) / 2, underline_y, len - underline_height(), underline_height() });
        ctx.fill({ (AppListView::icon_view_size() - len) / 2 + len, underline_y, underline_height(), underline_height() });
    } else if (entity().windows().size() > 0) {
        const int len = 10;
        ctx.fill({ (AppListView::icon_view_size() - len) / 2, underline_y, len, underline_height() });
    }
}
